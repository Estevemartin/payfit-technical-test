// For debugging pruposes in the interview I left some console.log().
// Since this is an optimization problem I didn't it was apropiate to use Jest for testing, since the result I expect might not be the best one.
...
class Oracle {
  ...
  public getPriceToPay() {
    return this.price_to_pay
  }
  public sortInhabitants(){
    // Puts the inhabitants that know less people in the first places.
    return this.village.sort((a,b):number=>{
      return(a.friends.length-b.friends.length)
    })
  }
  public getPossibleSuperStars(){
    return this.village.filter((person:Inhabitant):boolean=>{
  ...
  public getNormalInhabitants(){
    return this.village.filter((person:Inhabitant):boolean=>{
      // We do "==" and not "===" in case person.friends[0] is "0" (even though this should not happen if the rest of the code it's correct.)
      return(person.friends[0]!=0)
    })
  }
  public knows(personA: number, personB: number): boolean {
    this.price_to_pay += 1
    let persA = this.village.filter((pers:Inhabitant)=>pers.id===personA)[0]
    let persB = this.village.filter((pers:Inhabitant)=>pers.id===personB)[0]

    // console.log(`QUESTION ${this.price_to_pay}: Does`, persA, 
    // " knows:", persB,
    // "? -->", persA.friends.includes(persB.id))

    if(personA>this.village.length){return false}
    return persA.friends.includes(persB.id)
  }
  private parse_content(file_content: string[]): Inhabitant[] {
    const result: Inhabitant[] = []

    // Is this necessary?
    // const first_line = file_content[0].split(' ')
    // const size_of_the_village: number = +first_line[0]
    // Beeing able to do this?
    const size_of_the_village: number = +file_content[0]
    ...
    for (let i = 0; i <= size_of_the_village-1; ++i) {
      result[i] = new Inhabitant(
        i+1,
        file_content[i+1].split(' ').map((x: string) => +x)
      )
    }
    return result
  }
...
export function findSuperstar(oracle: Oracle): number[] {
  // Logical Approach:
  ...
  let possibleSuperStarsToIgnore: Inhabitant[] = []
  let alreadyCheckedFriendGroup: number[][] = []

  oracle.sortInhabitants()
  console.log("Village: ", oracle.getVillage())

  var possibleSuperStars = oracle.getPossibleSuperStars()
  if(possibleSuperStars.length===0){return[]}

  const normalInhabitants = oracle.getNormalInhabitants()
  // console.log("~ normalInhabitants", normalInhabitants)

  normalInhabitants.forEach((normalInhabitant:Inhabitant)=>{    
    possibleSuperStars.forEach((possibleSuperStar:Inhabitant)=>{
  ...
  if(possibleSuperStars===possibleSuperStarsToIgnore){return[]}
  const result = possibleSuperStars.filter((ps:Inhabitant)=>{
